...
// Obtener lista única de Rubros para selects/pickers
export const getRubrosUnicos = () => {
  const rubrosSet = new Set(RUBROS_DATA.map(item => item.rubro));
  return Array.from(rubrosSet).sort();
};

// Obtener detalles según el rubro seleccionado
export const getDetallesPorRubro = (rubroSeleccionado) => {
  return RUBROS_DATA
    .filter(item => item.rubro === rubroSeleccionado)
